...
(function (root, factory) {
		...
		return function (val, type, schema) {
			...
			val.forEach(function (subVal, subKey) {
				if (util.determineType(subVal) !== 'object') {
					this.throwError(name + '[' + subKey + '] must be a schema');
				}

				result[subKey] = new JsonSchema(subVal);
			});
			return result;
		};
	}
	...
	var allProperties = {
		...
		items: function (val, type, schema) {
			...
			val.forEach(function (subVal, subKey) {
				if (util.determineType(subVal) !== 'object') {
					this.throwError('items[' + subKey + '] must be a schema');
				}
		...
		definitions: function (val, type, schema) {
			...
			Object.keys(val).forEach(function (key) {
				if (util.determineType(val[key]) !== 'object') {
					this.throwError('definitions.' + key + ' must be a schema');
				}
	...
	JsonSchema.prototype.populateObject = function populateObject() {
		...
		if (!this.$ref) {
			...
			Object.keys(allProperties).forEach(function (key) {
				...
				if (val !== undefined) {
					this.debug('validate ' + key);
					result = allProperties[key].call(this, val, type, this.rawSchema);
					if (result !== undefined) {
						this.debug('validation success');
						this[getter] = result;
					} else {
						this.debug('validation returned nothing');
					}
				} else {
					this.debug('validate ' + key + ': not set');
				}
			});
		}
	};
